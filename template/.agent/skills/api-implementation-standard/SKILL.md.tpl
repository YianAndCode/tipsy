...
description: Strict guidelines for implementing backend APIs. Enforces the 3-layer architecture (Controller, Service, Repo), Wire dependency injection, naming conventions, and error handling patterns.
---
...
2.  **Strict Layering**: Never skip a layer. Even for simple CRUD, maintain the Controller -> Service -> Repository flow to ensure architectural integrity.
...
2.  **Service**: Contains core domain business logic.
3.  **Repository**: Handles data access.

### Naming Conventions

-   **Structs**: Must include the layer suffix: `Controller`, `Service`, or `Repo` (e.g., `UserController`, `UserService`, `UserRepo`).
...
1.  **File Location**: Define a controller struct for each module. The file should be located at `internal/controller/<module>/<module>_controller.go`.
    ...
    -   **Dependency Injection**: Use Wire to inject service instances into the controller struct.
...
3.  **Data Structures (Input/Output)**: If using an input/output struct, define it in a `data.go` file within the same service package directory (e.g., `internal/service/<module>/data.go`). This struct should only contain fields relevant to the domain logic.
...
5.  **Service Dependencies**: 
    ...
    -   **Solution**: 
        -   **Shared Utils**: If multiple services share common logic, extract it into a utility function under `internal/utils/`.
        -   **Domain Aggregation**: If two domain services are tightly bound (similar to a SQL LEFT JOIN), implement the combined logic in the side that represents the primary domain context.
        -   **Interfaces**: Define an interface in the `internal/contract` package and inject it into the Service constructor via Wire, if complex inter-domain calling is absolutely required.
...
5.  **Atomicity**: Ensure each repository function is atomic.
    -   **Transactions**: Do not initiate transactions within the repository. The caller (e.g., the Service layer) should manage transactions.
